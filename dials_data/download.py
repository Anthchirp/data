        ...
        def print(self, string):
            if verbose:
                ...
                print(
                    ...
                    * max(0, getattr(self, "last_line_length", 0) - len(string)),
                )
                sys.stdout.flush()
    ...
    class FileVerifier(threading.Thread):
        ...
        def run(self):
            while True:
                ...
                if (
                    os.path.exists(item["filename"])
                    and file_hash(item["filename"]) == item["checksum"]
                ):
    ...
    class FileDownloader(threading.Thread):
        ...
        def run(self):
            while True:
                ...
                success = True
                try:
                    _download_to_file(item["url"], item["filename"])
                    item["status"] = "Downloaded"
                    if file_hash(item["filename"]) != item["checksum"]:
        ...
        if target_dir:
            ...
            if os.getenv("REGRESSIONDATA"):
                ...
                self._target_dir = os.path.join(
                    os.getenv("LIBTBX_BUILD"), "regression_data"
                )
            else:
                raise RuntimeError(
                    "Could not determine regression data location. Use environment variable REGRESSIONDATA"
                )
    ...
    def __call__(self, test_data):
        if test_data not in self._cache:
            ...
            if self._cache[test_data]:
                self._cache[test_data] = str(
                    self._cache[test_data]
                )  # https://github.com/cctbx/cctbx_project/issues/234
